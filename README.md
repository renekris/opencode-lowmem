...
| Bounded root hot window      | The active lane's highest known root stays locally hot while you inspect nested subagents, so ordinary pressure no longer forces a reload when you return. Ordinary sessions evict first; an inactive root keeps only a coherent newest suffix and remains a final emergency eviction candidate                                                                                               | fixed root cap = `min(128 MiB, half the global payload budget)`; active roots keep the existing active-session policy; explicit return rehydrates an emergency-evicted root |
...
| TUI hot-path accounting      | Mirror-budget reconciliation batches each 120 ms event burst while preserving last-touch recency, and admitted text/reasoning deltas account their exact appended JSON bytes without serializing the growing part on every flush                                                                                                    | `packages/tui/src/context/data.tsx`, `mirror-budget.ts`, `payload-budget.ts`, and `sync.tsx`; full refresh, cap crossing, and authoritative replacement remain synchronous fallbacks |
...
| TUI root hot window | fixed `ROOT_HOT_WINDOW_MAX_BYTES` | `min(128 MiB, payload budget / 2)` | Inactive root's coherent message/part suffix; global payload budget `0` disables this fixed cap |
