...
| Child conveyor (daily TUI)    | Child sessions rank by their _newest_ inbound user message — a task continuation moves its child to the conveyor tail with the rest of the current wave instead of staying buried between stale agents. The child you are viewing stays pinned (footer stable; its queued messages apply as one re-rank when you leave). Children whose row events were missed are still discovered: `session.created` inserts directly and an unknown-row message triggers one row fetch | client-side rank map (first 256 distinct children, updated in place, never regressed, roots excluded); newest-50 window; viewed child pins with one deferred re-rank; single-flight session-row reconciliation on unknown-row messages; footer shows an honest N of 50 |
