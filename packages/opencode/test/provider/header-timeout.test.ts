...
it.live("chunkTimeout decodes multi-byte characters split across reads", () =>
  Effect.gen(function* () {
    const server = yield* Effect.acquireRelease(
      Effect.promise(() => utf8SplitBodyServer()),
      (server) => Effect.sync(() => server.server.close()),
    )
    ...
    yield* provideTmpdirInstance(
      () =>
        Effect.gen(function* () {
          ...
          expect(yield* Effect.promise(() => result.text)).toBe("õun — äля €")
        }),
...
async function utf8SplitBodyServer(): Promise<{ server: Server; url: string }> {
  const server = createServer((_, res) => {
    res.writeHead(200, { "content-type": "text/event-stream" })
    const payload = `data: ${JSON.stringify({ choices: [{ delta: { content: "õun — äля €" } }] })}\n\ndata: [DONE]\n\n`
