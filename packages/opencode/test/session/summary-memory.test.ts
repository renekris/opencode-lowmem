...
const it = testEffect(
  AppNodeBuilder.build(LayerNode.group([Session.node, SessionProjector.node, SessionSummary.node, Snapshot.node, CrossSpawnSpawner.node])),
)
...
describe("session summary memory retention", () => {
  it.instance(
    "stores lightweight summary diffs but still returns patches on demand",
    Effect.gen(function* () {
