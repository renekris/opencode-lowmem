...
export function createPartDeltaBuffer(options: { intervalMs?: number; apply: (entry: PartDeltaEntry) => void }) {
  ...
  const pending = new Map<string, PartDeltaEntry>()
  ...
  function flush() {
    ...
    pending.clear()
    for (const entry of entries) options.apply(entry)
  }
  ...
  return {
    push(input: { messageID: string; partID: string; field: string; delta: string }) {
      ...
      if (existing) existing.accumulated += input.delta
      else pending.set(key, { ...input, accumulated: input.delta })
